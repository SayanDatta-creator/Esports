    ...
    <Container maxWidth="md" className="news-container">
      <Box className="news-thread-list">
        {stickied.length > 0 && (
          <>
            {stickied.map(thread => (
              <Paper key={thread.id} className="news-thread-card stickied" elevation={4}>
                <Box className="news-thread-title-row">
                  <Typography variant="subtitle1" className="news-thread-title">
                    {thread.title} <Chip label="STICKIED" size="small" color="success" className="news-sticky-badge" />
                  </Typography>
                  <Typography variant="caption" className="news-thread-category">in {thread.category}</Typography>
                </Box>
                <Box className="news-thread-meta">
                  <span>by <b>{thread.author}</b></span>
